...
bool SDLGLGraphicsContext::Init(SDL_Window *&window, int x, int y, int mode, std::string *error_message) {
	...
	for (size_t i = 0; i < ARRAY_SIZE(attemptVersions); ++i) {
		...
		window = SDL_CreateWindow("Initializing GL...", x, y, pixel_xres, pixel_yres, mode);
		if (window == nullptr) {
			NativeShutdown();
	...
	if (glContext == nullptr) {
		...
		window = SDL_CreateWindow("Initializing GL (non-core)...", x,y, pixel_xres, pixel_yres, mode);
		if (window == nullptr) {
			NativeShutdown();
			fprintf(stderr, "SDL_CreateWindow failed: %s\n", SDL_GetError());
			SDL_Quit();
...
bool SDLVulkanGraphicsContext::Init(SDL_Window *&window, int x, int y, int mode, std::string *error_message) {
	...
	if (VK_SUCCESS != vulkan_->CreateInstance("PPSSPP", gitVer.ToInteger(), vulkanFlags)) {
		*error_message = vulkan_->InitError();
		delete vulkan_;
		vulkan_ = nullptr;
		return false;
